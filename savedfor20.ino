...
#define WIFI_PASSWORD "Abhi@2019"
#define API_KEY "AIzaSyBng7P4KNttbe6znCyJi66_mxc3NkLBpIs"
    ...
    if (Firebase.RTDB.get(&fbdo, path)) {
      ...
      if (fbdo.dataType() == "json") {
       ...
       if (json.get(jsonData, "dinner/clicked")) {
          ...
          if (dinnerClicked != vbState) { // Update yellow LED only if state changed
            vbState = dinnerClicked;
            // digitalWrite(yellow, yellowState ? HIGH : LOW);
            if (vbState) {
              vibration(); // Call blink function if yellow LED should be on
            }
          }
        }
      }
